				...
				if distance <= 200.0 then
					...
					if IsEntityAVehicle(vehicle) and GetPedInVehicleSeat(vehicle,-1) == ped then
						...
						if distance <= 12.0 then
							drawTxt("PRESSIONE  ~g~E~w~  PARA INICIAR A CORRIDA",4,0.5,0.93,0.50,255,255,255,180)
							if IsControlJustPressed(1,38) then
								if emP.checkPerm() then
									if cooldown <= 0 then
										racePos = 1
										inRace = true
										cooldown = 180
										...
										createBlipsRace()
										emP.startRace()
										TriggerEvent("Notify","sucesso","Você iniciou uma corrida na classe dos <b>Corredores</b>!",8000)
										TriggerEvent("Notify","aviso","Não saia do seu veículo, ou caso contrário, ele será explodido!",8000)
									else
										TriggerEvent("Notify","importante","Aguarde <b>"..parseInt(cooldown).." segundos</b> para iniciar uma nova corrida.",8000)
									end
								else
									TriggerEvent("Notify","importante","Você não é um <b>corredor</b> e não poderá participar das corridas.",8000)
