...
function vRP.dropPlayer(source)
	...
	if user_id then
		if user_id and source then
			...
			SendWebhookMessage(webhookexit,"```ini\n[ID]: "..user_id.." \n[IP]: "..GetPlayerEndpoint(source).." \n[SAIU DO SERVIDOR]"..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		end
		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("Você não tem whitelist em nossa cidade! Envie seu ID em nosso discord para que seja aprovado.\n\nDiscord: discord.gg/6JKXNnXJeS\nSeu ID: "..user_id)
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido da cidade.\n\nCaso seu banimento for injusto e deseja revogar ele, entre em contato em nosso suporte no Discord. \n\nDiscord: discord.gg/6JKXNnXJeS \nSeu ID: "..user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
