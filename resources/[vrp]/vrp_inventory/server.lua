...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhooklog_equipar = "" 
local webhooklog_enviar = ""
local webhooklog_dropar = ""
		...
		if type == "usar" then
			...
			elseif itemName == "colete" then
				if vRP.tryGetInventoryItem(user_id,"colete",1) then
					...
					TriggerClientEvent("Notify",source,"sucesso","<b>Colete</b> utilizado com sucesso.",8000)
		        end
...
-----------------------------------------------------------------------------------------------------------------------------------------
		elseif type == "equipar" then
			if vRP.tryGetInventoryItem(user_id,itemName,1) then
				...
				SendWebhookMessage(webhooklog_equipar,"```ini\n[ID]: "..user_id.." - "..identity.name.." "..identity.firstname.." \n[EQUIPOU]: "..vRP.itemNameList(itemName).." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
