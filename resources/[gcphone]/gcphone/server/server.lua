...
function src.rny_checandoItem()
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
        if vRP.getInventoryItemAmount(user_id,"celular") >= 1 or vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"platina.permissao") then
            return true 
        else
...
function serviceMessage(phone, sourcePlayer, message, type)
	...
	if user_id then
		...
		elseif phone == "juiz" then
			if inEmergency[user_id] == "juiz" then 
				TriggerClientEvent("Notify",source,"negado","Já Existe um chamado sendo averiguado!")
				return
			end
...
AddEventHandler('gcPhone:startCall',function(phone_number,rtcOffer,extraData)
	...
	elseif phoneNumber == "mechanic" then
		serviceMessage(phoneNumber, source, "", "call")
	elseif phoneNumber == "bennys" then
		serviceMessage(phoneNumber, source, "", "call")
	elseif phoneNumber == "ADM" then
		serviceMessage(phoneNumber, source, "", "call")
	else 
...
RegisterServerEvent('bank:transferaduoawl2oi21')
AddEventHandler('bank:transferaduoawl2oi21', function(id, amount)
    ...
    if targetPlayer == nil then
		...
		if tonumber(user_id) ~= tonumber(id) then
			if myBank >= amount then
				...
				vRP.logSytem(webhookbanco,"BANCO - CELULAR","```[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[ENVIOU PELO CELULAR]: $"..vRP.format(parseInt(amount)).."\n[PARA]:"..tonumber(id).."  "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			else
