...
Citizen.CreateThread(function()
	while true do
		...
		if distance <= 1.1 and not andamento then
			rsTD = 4
			drawTxt("PRESSIONE  ~g~E~w~  PARA HACKEAR AS CÂMERAS DE SEGURANÇA",4,0.5,0.93,0.50,255,255,255,180)
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(locais) do
			...
			if distance <= 1.1 and not andamento then
				rsTD = 4
				drawTxt("PRESSIONE  ~g~E~w~  PARA ROUBAR AS JOIAS",4,0.5,0.93,0.50,255,255,255,180)
				if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped) then
