...
function func.checkJewelry(x,y,z,h,sec,tipo)
	...
	if user_id then
		...
		elseif (os.time()-timers) <= 5400 then
			TriggerClientEvent("Notify",source,"aviso","A joalheria não se recuperou do ultimo roubo, aguarde <b>"..vRP.format(parseInt((5400-(os.time()-timers)))).." segundos</b> até que o sistema seja restaurado.")
		else
