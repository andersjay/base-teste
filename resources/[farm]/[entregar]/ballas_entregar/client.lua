...
emP = Tunnel.getInterface("ballas_entregar")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
local servico = false
local selecionado = 0
local CoordenadaX = 131.88
local CoordenadaY = -1937.96
local CoordenadaZ = 20.61
	...
	while true do
		...
		if servico then
			...
			if distance <= 10 then
				...
				if distance <= 1.2 then
					drawTxt("PRESSIONE  ~g~E~w~  PARA ENTREGAR AS DROGAS",4,0.5,0.93,0.50,255,255,255,180)
	...
	while true do
		...
		if servico then
			...
			if IsControlJustPressed(0,168) then
				servico = false
				RemoveBlip(blips)
				TriggerEvent("Notify","aviso","Você cancelou as entregas.")
			end
		end
		Wait(rsTD)
	end
